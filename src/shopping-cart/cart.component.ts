...
@Component({
  ...
  template: `
    <div>
      ...
      <button (click)="applyDiscount()" class="add-button discount">{{!isDiscountApplied?'💳 Apply Discount 10%':'💳 Remove Discount 10%'}}</button>
    </div>
  `,
})
export class CartComponent {
  ...
  applyDiscount() {
    if (!this.isDiscountApplied) {
      //  subtract 10%
      this.discountApplied += this.totalAmount * 0.1;
